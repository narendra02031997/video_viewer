...
class VideoViewerCore extends StatefulWidget {
  ...
  const VideoViewerCore({Key? key, required this.externalConnected,})
      : super(key: key);
...
class _VideoViewerCoreState extends State<VideoViewerCore> {
  ...
  void initState() {
    ...
    print("EXTERNAL CONNECTED ${widget.externalConnected}");
  ...
  Widget _player() {
    return Stack(children: [
        ...
        builder: (_, double scale, __) =>
            Transform.scale(
              ...
              child: VideoCorePlayer(
                externalConnected: widget.externalConnected,),
            ),
      ),
      ...
      VideoCoreForwardAndRewindLayout(
        rewind: GestureDetector(onTap: _rewind),
        forward: GestureDetector(onTap: _forward),
      ),
      Builder(builder: (_) {
        ...
        return Stack(children: [
          ...
          if (metadata.enableShowReplayIconAtVideoEnd)
            CustomOpacityTransition(
              ...
              child: const Center(
                child: PlayAndPause(
                  type: PlayAndPauseType.center, fromWhere: 2,),
              ),
            ),
        ]);
      }),
